...
# Phonological Tokenizer related (arXiv:2601.19781, Sony/Phonological-Tokenizer):
# Stage 3 discretizes speech with this fine-tuned WavLM-large + fixed
# released codebook instead of self-training a kmeans model. km_dir above
# must point at the km model converted from its centroids (see
# local/convert_phonological_centroids.py), and nclusters must be 2000.
phonological_tokenizer_ssl_path=downloads/phonological_tokenizer/ssl.pth
...
Options:
    ...
    # Kmeans related
    --km_dir            # Path to the pretrained km model (converted Phonological Tokenizer centroids)
    ...
    --phonological_tokenizer_ssl_path # Path to the fine-tuned WavLM-large checkpoint (default="${phonological_tokenizer_ssl_path}").
    --phonological_tokenizer_layer    # SSL layer to discretize (default="${phonological_tokenizer_layer}").
...
# path.sh/cmd.sh must be sourced before pyscripts/utils/print_args.py below:
# that script's shebang is "#!/usr/bin/env python" (not python3), which only
# resolves once path.sh's conda activation puts a "python" on PATH.
. ./path.sh
. ./cmd.sh
...
if [ -z "${km_dir}" ]; then
    km_dir="${expdir}"/kmeans/phonological_tokenizer_${nclusters}clusters
fi
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ] && ! [[ " ${skip_stages} " =~ [[:space:]]3[[:space:]] ]]; then
    if "${use_speech}"; then
        log "Stage 3a: Discretize speech with the Phonological Tokenizer (layer ${phonological_tokenizer_layer})"

        # Both steps are idempotent: they skip their own work if the
        # download/conversion was already done in a previous run.
        _phonological_tokenizer_dir=$(dirname "${phonological_tokenizer_ssl_path}")
        local/download_phonological_tokenizer.sh --download_dir "${_phonological_tokenizer_dir}"
        python3 local/convert_phonological_centroids.py \
            --centroids_path "${_phonological_tokenizer_dir}/centroids.npy" \
            --km_path "${km_dir}/km_${nclusters}.mdl" \
            --n_clusters "${nclusters}"

        phonological_tokenizer_conf="{type=phonological_tokenizer,conf={ssl_model_path=${phonological_tokenizer_ssl_path},layer=${phonological_tokenizer_layer}}}"

        for _dir in "data/${train_set}/speech/"*; do
            if [ -d "${_dir}" ]; then
                scripts/feats/perform_kmeans.sh \
                    ...
                    --feature_type phonological_tokenizer \
                    --layer "${phonological_tokenizer_layer}" \
                    --feature_conf "${phonological_tokenizer_conf}" \
                    --km_dir "${km_dir}" \
                    --portion "${portion}" \
                    --nclusters "${nclusters}" \
        ...
        _suf="layer${phonological_tokenizer_layer}/"

        for dset in "${train_set}" "${valid_set}" ${test_sets}; do
            for _dir in "data/${dset}/speech/"*; do
                if [ -d "${_dir}" ]; then
                    utils/copy_data_dir.sh "${data_audio}/$(basename ${_dir})/${dset}" "${data_feats}/${dset}/speech/$(basename ${_dir})"
                    cat "${data_extract}/$(basename ${_dir})/phonological_tokenizer/${_suf}${dset}/pseudo_labels_km${nclusters}.txt" \
